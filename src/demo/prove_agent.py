...
class ClientNaiveAgent(Thread):
    ...
    def read_test_result(self, file_name):
        ...
        shoot_angles = []
        while line:
            shoot_angles.append(line.split(" ")[0])
            line = file_name.readline()
        file_name.close()
        return shoot_angles
    ...
    def run(self):
        ...
        while True:
            ...
            answer_file = open(self.result_path + str(self.current_level) + ".txt")
            angle = self.read_test_result(answer_file)
            if self.current_bird < len(angle):
                state = self.solve(int(angle[self.current_bird]))
            else:
                state = self.solve(int(random.choice(angle)))
            self.current_bird += 1
